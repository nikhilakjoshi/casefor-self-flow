...
import { streamStrengthEvaluation } from "@/lib/strength-evaluation"
...
export async function POST(
  ...
  const streamResult = await streamStrengthEvaluation(caseId)
  ...
  const stream = new ReadableStream({
    async start(controller) {
      try {
        ...
        console.error("Strength evaluation stream error:", err)
  ...
  streamResult.output.then(async (output) => {
    ...
    try {
      ...
      console.error("Failed to save strength evaluation:", err)
    }
  })
