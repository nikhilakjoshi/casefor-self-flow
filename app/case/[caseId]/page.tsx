...
export default async function CasePage({ params }: Props) {
  ...
  const [latestStrengthEval, latestGapAnalysis, latestCaseStrategy, latestCaseConsolidation, latestDenialProbability] = await Promise.all([
    ...
    db.caseConsolidation.findFirst({
      where: { caseId },
      orderBy: { createdAt: 'desc' },
      select: { data: true },
    }),
  ...
  return (
    <CasePageClient
      ...
      initialCaseConsolidation={latestCaseConsolidation?.data as import('@/lib/case-consolidation-schema').CaseConsolidation | null ?? null}
      initialDenialProbability={latestDenialProbability?.data as import('@/lib/denial-probability-schema').DenialProbability | null ?? null}
    />
  )
}
